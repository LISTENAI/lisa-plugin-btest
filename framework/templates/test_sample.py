...
#@pytest.mark.parametrize('connected_device', ['/path/to/your/device-map.yml'], indirect=True)
                                       #如果device-map.yml不在生成时的默认位置，请解除注释此装饰器并在/path/to/your/device.yml处指定路径。
                                       #Uncomment this decorator and specify the path of your device-map.yml if it is not placed in default location when generated.
class Test_01:
#   @pytest.mark.usefixtures("reset")  #如果**仅需在此测试用例**运行前进行模组复位，请解除注释此装饰器。
    ...
    def test_1(self, connected_device):
    ...
    def test_yourcase(self, connected_device):
        (shell) = connected_device
        # 您的测试用例代码
        # Your custom test case code here
        assert True
